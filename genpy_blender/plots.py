...
#        self.axes.end = [ex + s for ex, s in zip(self.axes.extent, self.axes.start)]
        x = ((x - self.axes.start[0]) * (self.axes.axis_end[0] - self.axes.axis_start[0]) / (self.axes.end[0] - self.axes.start[0]))
        y = ((y - self.axes.start[1]) * (self.axes.axis_end[1] - self.axes.axis_start[1]) / (self.axes.end[1] - self.axes.start[1]))
        z = ((z - self.axes.start[2]) * (self.axes.axis_end[2] - self.axes.axis_start[2]) / (self.axes.end[2] - self.axes.start[2]))
        return x, y, z
